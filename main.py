...
async def ws_endpoint(websocket: WebSocket, room_code: str, player_name: str, token: Optional[str] = None):
    ...
    async def _cleanup_desconexion():
        ...
        if player.ws is not websocket:
            # Esta conexión ya fue reemplazada por una más nueva (la persona
            # recargó o se reconectó) antes de que el servidor se diera
            # cuenta de que ESTA en particular se había cortado — algo
            # frecuente en celulares, donde el sistema operativo puede
            # tardar varios segundos en avisar que una conexión de fondo
            # murió. Sin este chequeo, cuando por fin llega ese aviso tardío
            # se pisaba el estado del jugador (player.connected = False)
            # aunque la conexión nueva siguiera funcionando perfecto,
            # dejando la partida "trabada" unos segundos después de cada
            # reconexión hasta que la persona recargaba de nuevo a mano.
            return
        player.connected = False
...
async def ws_batalla_endpoint(websocket: WebSocket, room_code: str, player_name: str, token: Optional[str] = None):
    ...
    async def _cleanup_batalla():
        if player.ws is not websocket:
            # Ver el comentario equivalente en _cleanup_desconexion (mismo
            # bug, misma corrección): esta conexión ya fue reemplazada por
            # una más nueva antes de que el servidor notara que ESTA se
            # había cortado, así que no hay que tocar el estado del jugador.
            return
        player.connected = False
        room._sync_disconnect_timer()
        try:
